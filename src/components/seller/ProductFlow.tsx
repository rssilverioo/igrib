...
const ProductFlow = () => {
  ...
  const handleFiles = (files: File[]) => {
    ...
    const validFiles = files.filter(file => {
      if (file.size > MAX_FILE_SIZE) {
        setUploadError(t('seller.products.productFlow.images.sizeError'));
        return false;
      }
      return true;
    });
  ...
  const removeImage = (index: number) => {
    URL.revokeObjectURL(product.imagePreviews[index]);
    setProduct({
      ...product,
      images: product.images.filter((_, i) => i !== index),
      imagePreviews: product.imagePreviews.filter((_, i) => i !== index)
    });
  };

  const uploadImageToTigris = async (file: File): Promise<string> => {
    // Get presigned URL
    const { uploadUrl, key } = await api.post<{ uploadUrl: string; key: string; publicUrl: string }>('/upload/presigned-url', {
      fileName: file.name,
    ...
    if (step === steps.length) {
      ...
      try {
        ...
        const newProduct = await createProduct({
          ...
          quantity: parseInt(product.available),
          unit: 'saca',
        ...
        // 2. Upload images to Tigris and save records
        for (let i = 0; i < product.images.length; i++) {
          const file = product.images[i];
          const key = await uploadImageToTigris(file);
          await api.post(`/products/${newProduct.id}/images`, {
            url: `${process.env.NEXT_PUBLIC_TIGRIS_ENDPOINT_URL || 'https://t3.storage.dev'}/${process.env.NEXT_PUBLIC_TIGRIS_BUCKET_NAME || 'igrib'}/${key}`,
            key,
            order: i,
          });
        }
      ...
      case 2:
        return (
          ...
          >
            ...
            >
              {[
                { key: 'humidity', icon: Droplet, placeholder: '14%' },
                { key: 'protein', icon: Scale, placeholder: '40%' },
                { key: 'purity', icon: Package, placeholder: '99%' },
                { key: 'certification', icon: Certificate, placeholder: 'Organico BR' },
              ].map((spec) => (
                <motion.div
                  key={spec.key}
                ...
                >
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <label className="block text-sm font-medium text-gray-700">
                      {t(`seller.products.productFlow.specifications.${spec.key}`)}
                    </label>
                  </div>
                  <input
                    type="text"
                    value={product.specifications[spec.key as keyof typeof product.specifications]}
                    onChange={(e) => setProduct({
                      ...product,
                      specifications: { ...product.specifications, [spec.key]: e.target.value }
                    })}
                    className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
      ...
      case 3:
        return (
          ...
          >
            ...
            <div className="space-y-4">
              <motion.div
                className={`border-2 border-dashed rounded-lg p-6 sm:p-8 transition-colors cursor-pointer ${
              ...
              >
                <div className="text-center">
                  <Upload className={`h-10 w-10 sm:h-12 sm:w-12 mx-auto mb-4 ${isDragging ? 'text-green-600' : 'text-gray-400'}`} />
              ...
              >
                <motion.div
                  className={`p-2 rounded-full ${
                    index <= step - 1 ? 'bg-green-100' : 'bg-gray-100'
                  }`}
                  animate={{ scale: index === step - 1 ? [1, 1.1, 1] : 1 }}
